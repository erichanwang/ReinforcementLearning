...
import Helpers.testClasses
import random, math, traceback, sys, os
import Helpers.layout, Helpers.textDisplay, pacman, gridworld
import time
from Helpers.util import Counter, TimeoutFunction, FixedRandom, Experiences
...
class ValueIterationTest(Helpers.testClasses.TestCase):
    ...
    def runAgent(self, moduleDict, numIterations):
        agent = moduleDict['Helpers/valueIterationAgents'].ValueIterationAgent(self.grid, discount=self.discount, iterations=numIterations)
...
class AsynchronousValueIterationTest(ValueIterationTest):
    def runAgent(self, moduleDict, numIterations):
        agent = moduleDict['Helpers/valueIterationAgents'].AsynchronousValueIterationAgent(self.grid, discount=self.discount, iterations=numIterations)
        states = self.grid.getStates()
        actions = list(reduce(lambda a, b: set(a).union(b), [self.grid.getPossibleActions(state) for state in states]))
        values = {}
        qValues = {}
...
class PrioritizedSweepingValueIterationTest(ValueIterationTest):
    def runAgent(self, moduleDict, numIterations):
        agent = moduleDict['Helpers/valueIterationAgents'].PrioritizedSweepingValueIterationAgent(self.grid, discount=self.discount, iterations=numIterations)
        states = self.grid.getStates()
        actions = list(reduce(lambda a, b: set(a).union(b), [self.grid.getPossibleActions(state) for state in states]))
        values = {}
        qValues = {}
...
class ApproximateQLearningTest(Helpers.testClasses.TestCase):
...
class QLearningTest(Helpers.testClasses.TestCase):
    ...
    def runAgent(self, moduleDict, numExperiences):
        agent = moduleDict['Exercises/qlearningAgents'].QLearningAgent(**self.opts)
        states = [state for state in self.grid.getStates() if len(self.grid.getPossibleActions(state)) > 0]
        states.sort()
        lastExperience = None
        for i in range(numExperiences):
...
class EpsilonGreedyTest(Helpers.testClasses.TestCase):
...
### q8
class Question8Test(Helpers.testClasses.TestCase):

    def __init__(self, question, testDict):
        super(Question8Test, self).__init__(question, testDict)

    def execute(self, grades, moduleDict, solutionDict):
        studentSolution = moduleDict['Exercises/analysis'].question8()
...
class EvalAgentTest(Helpers.testClasses.TestCase):
...
def computePolicy(moduleDict, grid, discount):
    valueIterator = moduleDict['Helpers/valueIterationAgents'].ValueIterationAgent(grid, discount=discount)
    policy = {}
...
class GridPolicyTest(Helpers.testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        if not hasattr(moduleDict['Exercises/analysis'], self.parameterFn):
            self.addMessage('Method not implemented: analysis.%s' % (self.parameterFn,))
            return self.testFail(grades)

        result = getattr(moduleDict['Exercises/analysis'], self.parameterFn)()
