...
class Gridworld(Helpers.mdp.MarkovDecisionProcess):
    """
      Gridworld
    """
...
class GridworldEnvironment(Helpers.environment.Environment):
...
def getUserAction(state, actionFunction):
    ...
    """
    import Helpers.graphicsUtils
    action = None
    while True:
        keys = Helpers.graphicsUtils.wait_for_keys()
...
if __name__ == '__main__':
    ...
    import Helpers.textGridworldDisplay
    display = Helpers.textGridworldDisplay.TextGridworldDisplay(mdp)
    if not opts.textDisplay:
        import Helpers.graphicsGridworldDisplay
        display = Helpers.graphicsGridworldDisplay.GraphicsGridworldDisplay(mdp, opts.gridSize, opts.speed)
    try:
        display.start()
    ...
    import Helpers.valueIterationAgents, Exercises.qlearningAgents
    a = None
    if opts.agent == 'value':
        a = Helpers.valueIterationAgents.ValueIterationAgent(mdp, opts.discount, opts.iters)
    elif opts.agent == 'q':
        ...
        qLearnOpts = {'gamma': opts.discount,
                      ...
                      'actionFn': actionFn}
        a = Exercises.qlearningAgents.QLearningAgent(**qLearnOpts)
        ...
        a = Helpers.valueIterationAgents.AsynchronousValueIterationAgent(mdp, opts.discount, opts.iters)
    elif opts.agent == 'priosweepvalue':
        a = Helpers.valueIterationAgents.PrioritizedSweepingValueIterationAgent(mdp, opts.discount, opts.iters)
    else:
        ...
        if not opts.manual and opts.agent in ('value', 'asynchvalue', 'priosweepvalue'):
            if opts.valueSteps:
                for i in range(opts.iters):
                    tempAgent = Helpers.valueIterationAgents.ValueIterationAgent(mdp, opts.discount, i)
